...
import { toPastel } from "../utils/colors";

// helpers for representing flags that have different sized stripes
const sizedStripe = (color: string, size: number) => {
  return {
    color,
    size,
  };
...
const toSizedStripe = (stripe: any) => {
...
const pastelizeStripe = ({ color, size }: { color: string; size: number }) => {
  return {
...
const toSizedPastelStrip = (stripe: any) =>
  pastelizeStripe(toSizedStripe(stripe));
